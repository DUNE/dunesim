////////////////////////////////////////////////////////////////////////
/// \file  CORSIKAGendp_module.cc
/// \brief Generator for cosmic-ray secondaries based on pre-generated CORSIKA shower databases.
/// \Basic a copy of larsim/EventGenerator/CORSIKA/CORSIKAGen_module.cc by mike bass adapted for Dual-Phase purposes
/// \author  ascarpel@apc.in2p3.fr
////////////////////////////////////////////////////////////////////////
//#ifndef EVGEN_CORSIKAGendp_H
//#define EVGEN_CORSIKAGendp_H
...
namespace evgendp{
  ...
  class CORSIKAGendp : public art::EDProducer {
  public:
    explicit CORSIKAGendp(fhicl::ParameterSet const& pset);
...
namespace evgendp{
      ...
      fDualphase(p.get< bool >("DoRotation"))
  {
    ...
    if(fShowerInputFiles.size() != fShowerFluxConstants.size() || fShowerInputFiles.size()==0 || fShowerFluxConstants.size()==0)
      throw cet::exception("CORSIKAGendp") << "ShowerInputFiles and ShowerFluxConstants have different or invalid sizes!"<<"\n";
    fShowerInputs=fShowerInputFiles.size();

    if(fSampleTime==0.) throw cet::exception("CORSIKAGendp") << "SampleTime not set!";
  ...
  void CORSIKAGendp::DoRotation(double vec[] ){
    //rotate a vector of 90^ around z (ProtoDUNE s.o.r.)
    double dummyvec[3];
    dummyvec[0] = vec[0];
    dummyvec[1] = vec[1];
    dummyvec[2] = vec[3];
    vec[0] = dummyvec[1];   // x' = y
    vec[1] = -dummyvec[0]; //y' = -x
    vec[2] = dummyvec[2]; //z' = z

    return;
  }

  void CORSIKAGendp::ProjectToBoxEdge(	const double 	xyz[],
                                        const double 	indxyz[],
                                        const double 	xlo,
                                        const double 	xhi,
                                        const double 	ylo,
  ...
  double CORSIKAGendp::wrapvarBoxNo( const double var, const double low, const double high, int& boxno){
    //wrap variable so that it's always between low and high
    boxno=int(floor(var/(high-low)));
    return (var - (high - low) * floor(var/(high-low))) + low;
  }

  void CORSIKAGendp::populateTOffset(){
    ...
    for(int i=0; i<fShowerInputs; i++){
        ...
        if ( sqlite3_prepare(fdb[i], kStatement.c_str(), -1, &statement, 0 ) == SQLITE_OK ){
          ...
          if ( res == SQLITE_ROW ){
            ...
            mf::LogInfo("CORSIKAGendp")<<"For showers input "<< i<<" found particles.min(t)="<<t<<"\n";
            if (i==0 || t<fToffset_corsika) fToffset_corsika=t;
          }else{
            throw cet::exception("CORSIKAGendp") << "Unexpected sqlite3_step return value: (" <<res<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
          }
        }else{
          throw cet::exception("CORSIKAGendp") << "Error preparing statement: (" <<kStatement<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
        }
    }

    mf::LogInfo("CORSIKAGendp")<<"Found corsika timeoffset [ns]: "<< fToffset_corsika<<"\n";
  }

  void CORSIKAGendp::populateNShowers(){
    ...
    */
    ...
    for(int i=0; i<fShowerInputs; i++){
      ...
      //  double thisrnd=flat();//need a new random number for each query
        if ( sqlite3_prepare(fdb[i], kStatement.c_str(), -1, &statement, 0 ) == SQLITE_OK ){
          ...
          if ( res == SQLITE_ROW ){
            ...
            mf::LogVerbatim("CORSIKAGendp")<<"For showers input "<< i<<" found e_hi="<<upperLimitOfEnergyRange<<", e_lo="<<lowerLimitOfEnergyRange<<", slope="<<energySlope<<", k="<<fShowerFluxConstants[i]<<"\n";
          }else{
            throw cet::exception("CORSIKAGendp") << "Unexpected sqlite3_step return value: (" <<res<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
          }
        }else{
      ...
      mf::LogVerbatim("CORSIKAGendp")<<"For showers input "<< i
                               <<" the number of showers per event is "<<(int)NShowers<<"\n";
    }
  }

  void CORSIKAGendp::GetSample(simb::MCTruth& mctruth){
    ...
    int shower,pdg;
    double px,py,pz,x=0, y=0,z,tParticleTime,etot,showerTime=0.,showerTimex=0.,showerTimez=0.,showerXOffset=0.,showerZOffset=0.,t;
    for(int i=0; i<fShowerInputs; i++){
      nShowerCntr=randpois.fire(fNShowersPerEvent[i]);
      mf::LogInfo("CORSIKAgendp") << " Shower input " << i << " with mean " << fNShowersPerEvent[i] << " generating " << nShowerCntr;
      ...
      while(nShowerCntr>0){
        ...
        //    }
          ...
          throw cet::exception("CORSIKAGendp") << "Error preparing statement: (" <<kthisStatement<<"); "<<"ERROR:"<<sqlite3_errmsg(fdb[i])<<"\n";
        }
  ...
  void CORSIKAGendp::reconfigure(fhicl::ParameterSet const& p){

    return;
  }

  void CORSIKAGendp::beginJob(){
    art::ServiceHandle<art::TFileService> tfs;


  }

  void CORSIKAGendp::beginRun(art::Run& run){
  ...
  void CORSIKAGendp::produce(art::Event& evt){
    ...
    mf::LogInfo("CORSIKAGendp")<<"Number of particles from getsample crossing cryostat + bounding box: "<<truth.NParticles()<<"\n";
...
namespace evgendp{
  ...
  DEFINE_ART_MODULE(CORSIKAGendp)

}

//#endif // EVGEN_CORSIKAGendp_H
////////////////////////////////////////////////////////////////////////
